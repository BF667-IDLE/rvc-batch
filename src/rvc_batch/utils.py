...
from rvc_batch.config.variable import SAMPLE_RATE, PREDICTOR_MODEL_DICT, HF_PREDICTOR_BASE, HF_EMBEDDER_BASE, AUTOTUNE_SCALES, MODELS_DIR
...
def clear_gpu_cache():
    ...
    if torch.cuda.is_available(): torch.cuda.empty_cache()
        ...
        try:
            import opencl
            if opencl.is_available(): opencl.pytorch_ocl.empty_cache()
        except ImportError:
            pass
...
def check_predictors(method):
    def download(predictors):
        dest = os.path.join(MODELS_DIR, predictors)
        if not os.path.exists(dest): 
           os.makedirs(MODELS_DIR, exist_ok=True)
           HF_download_file(HF_PREDICTOR_BASE + predictors, dest)
...
def check_embedders(hubert="hubert_base"):
    if hubert in ["hubert_base"]:
        hubert += ".pt"
        model_path = os.path.join(MODELS_DIR, hubert)
