...
from rvc_batch.config.variable import SAMPLE_RATE as SAMPLE_RATE, DJCM_WINDOW_LENGTH as WINDOW_LENGTH, DJCM_N_CLASS as N_CLASS
...
class _ResEncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_blocks, kernel_size):
        ...
        self.conv = nn.ModuleList([_ResConvBlock(in_channels, out_channels)])
        for _ in range(n_blocks - 1):
            self.conv.append(_ResConvBlock(out_channels, out_channels))
...
class _Encoder(nn.Module):
    def __init__(self, in_channels, n_blocks):
        ...
        self.en_blocks = nn.ModuleList([
            _ResEncoderBlock(in_channels, 32, n_blocks, (1, 2)),
            _ResEncoderBlock(32, 64, n_blocks, (1, 2)),
            _ResEncoderBlock(64, 128, n_blocks, (1, 2)),
            _ResEncoderBlock(128, 256, n_blocks, (1, 2)),
            _ResEncoderBlock(256, 384, n_blocks, (1, 2)),
            _ResEncoderBlock(384, 384, n_blocks, (1, 2)),
        ])
...
class _ResDecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_blocks, stride):
        ...
        self.conv = nn.ModuleList([_ResConvBlock(out_channels * 2, out_channels)])
        for _ in range(n_blocks - 1):
            self.conv.append(_ResConvBlock(out_channels, out_channels))
        self.init_weights()
...
class _Decoder(nn.Module):
    def __init__(self, n_blocks):
        ...
        self.de_blocks = nn.ModuleList([
            _ResDecoderBlock(384, 384, n_blocks, (1, 2)),
            _ResDecoderBlock(384, 384, n_blocks, (1, 2)),
            _ResDecoderBlock(384, 256, n_blocks, (1, 2)),
            _ResDecoderBlock(256, 128, n_blocks, (1, 2)),
            _ResDecoderBlock(128, 64, n_blocks, (1, 2)),
            _ResDecoderBlock(64, 32, n_blocks, (1, 2)),
        ])
...
class _PE_Decoder(nn.Module):
    def __init__(self, n_blocks, seq_layers=1, window_length=1024, n_class=360):
        super(_PE_Decoder, self).__init__()
        self.de_blocks = _Decoder(n_blocks)
        self.after_conv1 = _ResEncoderBlock(32, 32, n_blocks, None)
        self.after_conv2 = nn.Conv2d(32, 1, (1, 1))
        self.fc = nn.Sequential(
            _BiGRU((1, window_length // 2), 1, seq_layers),
            nn.Linear(window_length // 2, n_class),
            nn.Sigmoid(),
        )
        _init_layer(self.after_conv2)

    def forward(self, x, concat_tensors):
        return self.fc(self.after_conv2(self.after_conv1(self.de_blocks(x, concat_tensors)))).squeeze(1)


class _SVS_Decoder(nn.Module):
    def __init__(self, in_channels, n_blocks):
        super(_SVS_Decoder, self).__init__()
        self.de_blocks = _Decoder(n_blocks)
        self.after_conv1 = _ResEncoderBlock(32, 32, n_blocks, None)
...
class _LatentBlocks(nn.Module):
    def __init__(self, n_blocks, latent_layers):
        super(_LatentBlocks, self).__init__()
        self.latent_blocks = nn.ModuleList([
            _ResEncoderBlock(384, 384, n_blocks, None)
...
class DJCMM(nn.Module):
    def __init__(self, in_channels, n_blocks, latent_layers, svs=False, window_length=1024, n_class=360):
        ...
        self.pe_encoder = _Encoder(in_channels, n_blocks)
        self.pe_latent = _LatentBlocks(n_blocks, latent_layers)
        self.pe_decoder = _PE_Decoder(n_blocks, window_length=window_length, n_class=n_class)
        self.svs = svs
        if svs:
            self.svs_encoder = _Encoder(in_channels, n_blocks)
            self.svs_latent = _LatentBlocks(n_blocks, latent_layers)
            self.svs_decoder = _SVS_Decoder(in_channels, n_blocks)
        _init_bn(self.bn)
    ...
    def forward(self, audio, center=True):
        ...
        if str(audio.device).startswith(("ocl", "privateuseone")):
            if not hasattr(self, "stft"):
                from rvc_batch.library.backends.utils import STFT
                self.stft = STFT(
...
class DJCM:
    def __init__(
        ...
        if self.onnx:
            ...
            model = DJCMM(1, 1, 1, svs=svs, window_length=win_len, n_class=N_CLASS)
        ...
        self.spec_extractor = _Spectrogram(int(SAMPLE_RATE // 100), win_len).to(device)
        cents_mapping = 20 * np.arange(N_CLASS) + 1997.3794084376191
        self.cents_mapping = np.pad(cents_mapping, (4, 4))

    def spec2hidden(self, spec):
        if self.onnx:
            spec_np = spec.cpu().numpy().astype(np.float32)
            hidden = torch.as_tensor(
                self.model.run(
                    [self.model.get_outputs()[0].name],
                    {self.model.get_inputs()[0].name: spec_np},
                )[0],
                device=self.device,
            )
        else:
